...
const { s3Client, MARKERS_BUCKET_NAME, MANIFEST_KEY, CODE_VERSION } = require('./config');
...
async function updateManifest(filesProcessed, discoveredAgents, lastProcessedTimestamp, failedFiles = [], failedMessages = []) {
    try {
        ...
        const manifest = {
            // Schema version of this file, not of the code — the two move independently.
            version: '2.4',
            // Which build wrote this checkpoint. With the updater rolling versions out on
            // its own, this is the record of what was actually running at the time.
