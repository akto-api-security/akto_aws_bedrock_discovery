...
async function addAgentRoleAndPermissions(tags, agentId, knownRoleArn) {
    try {
        ...
        return {
            ...tags,
            'bedrock-execution-role-arn': roleArn,
            'bedrock-execution-role': roleName,
            ...await getRoleSecurityProfile(roleArn, 'bedrock')
        };
    } catch (error) {
...
async function createStandardMessage(pair) {
    ...
    if (pair.logType === 'AGENT' && pair.agentId) {
        ...
        const callerRoleArn = `arn:aws:iam::${pair.accountId || AWS_ACCOUNT_ID}:role/${roleName}`;
        agentTags = await fetchTagsCached(`caller-enriched-${roleName}`, async () => ({
            'bedrock-execution-role-arn': callerRoleArn,
            'bedrock-execution-role': roleName,
            // An IAM_USER caller has no role to profile; only a role is readable here,
            // and the profile supplies bedrock-role-policies for the IAM_ROLE case.
            'bedrock-role-policies': '',
            ...(pair.callerKind === 'IAM_ROLE' ? await getRoleSecurityProfile(callerRoleArn, 'bedrock') : {})
        }));
        awsMetadata = {
            model: pair.modelId,
            'bedrock-execution-role': roleName,
